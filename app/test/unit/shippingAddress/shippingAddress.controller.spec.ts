...
describe('ShippingAdress Controller', () => {
  ...
  it('POST should return 201 when post data is Ok', async () => {
    ...
    expect(returnedValue).toEqual(mockShippingAddress.shippingAddressEntity[0]);
  });
  ...
  it('GET should return 200 when get shipping address is OK', async () => {
    ...
    const returnedValue = await shippingAddressController.findShippingAddress(page, size, orderBy, sortBy, uid);
  ...
  it('GET should return 403 when it cannnot retrieve data', async () => {
    ...
    try {
      await shippingAddressController.findShippingAddress(page, size, orderBy, sortBy, uid);
    } catch (error) {
      expect(error).toBeInstanceOf(HttpException);
