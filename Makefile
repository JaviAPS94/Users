...
USERS_REPOSITORY_NAME ?= ecommerce-ms-users
...
docker-build-users:
	@./scripts/docker_apply.sh "build" ${ECR_REPOSITORY} ${USERS_REPOSITORY_NAME} "docker/Dockerfile"

docker-export-users:
	@./scripts/docker_apply.sh "export" ${ECR_REPOSITORY} ${USERS_REPOSITORY_NAME}
...
docker-package-users:
	@./scripts/docker_apply.sh "package" $(ECR_REPOSITORY) ${USERS_REPOSITORY_NAME} $(VERSION)

docker-publish-users:
	@./scripts/docker_apply.sh "publish" $(ECR_REPOSITORY) ${USERS_REPOSITORY_NAME} $(VERSION)

local-run:
	@./scripts/local_apply.sh "run" "${ECR_REPOSITORY}/${USERS_REPOSITORY_NAME}:${VERSION}"
...
create-migration:
	@./scripts/run_migrations.sh "run" "${ECR_REPOSITORY}/${USERS_REPOSITORY_NAME}:${VERSION}" "generate -- -n ${FILENAME}"

run-migration:
	@./scripts/run_migrations.sh "run" "${ECR_REPOSITORY}/${USERS_REPOSITORY_NAME}:${VERSION}" "run"

run-code-analysis:
	@./scripts/code_analysis_apply.sh "run-sonar" "${SONAR_HOST_URL}" "${SONAR_TOKEN}"

run-unit-tests:
	@./scripts/test_apply.sh "run-unit" "${ECR_REPOSITORY}/${USERS_REPOSITORY_NAME}:${VERSION}"

run-e2e-tests:
	@./scripts/test_apply.sh "run-e2e" "${ECR_REPOSITORY}/${USERS_REPOSITORY_NAME}:${VERSION}"
